...
<AccordionGroup>
  <Accordion title="Plain English explanation">
    ...
    - Platforms you haven't connected (M365, Google Workspace) are excluded entirely, so you're only scored on what's relevant to your environment
  </Accordion>
...
Score history is per-assessment and shows how the client's posture has changed over time, which is useful for demonstrating remediation progress and justifying ongoing monitoring fees.
...
<AccordionGroup>
  ...
  <Accordion title="Will the scoring model change over time?">
    Yes. Telivy periodically updates the scoring engine. Changes to severity weights, normalization logic, or control mappings may shift scores. These updates are always aimed at making the score a more accurate reflection of actual risk, and changes are communicated in release notes.
  </Accordion>
</AccordionGroup>
