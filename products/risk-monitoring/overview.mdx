...
This setting controls how often the client's computers are fully rescanned. Some checks run on their own schedule no matter what you pick here: everything Telivy checks from the outside is refreshed weekly, and connected Microsoft 365 and Google Workspace tenants are read daily. [Scanning Schedule](/products/risk-monitoring/scanning-schedule) explains what is checked when, and why findings sometimes change between rescans.
...
<AccordionGroup>
  ...
  <Accordion title="Why did findings change between rescans?">
    Some checks run on their own schedule. The outside view of the client refreshes weekly, connected cloud tenants are read daily, and newly published vulnerabilities are compared against the software Telivy already knows about weekly, so findings can appear without any device being touched. [Scanning Schedule](/products/risk-monitoring/scanning-schedule) breaks it down.
  </Accordion>
