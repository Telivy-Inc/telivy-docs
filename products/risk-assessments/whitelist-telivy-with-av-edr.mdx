...
1. When using RMM installation, the first step is to download the MSI from [install.telivy.com](http://install.telivy.com) and install package. If you notice that the download has failed, please check your anti virus status (instructions seen above).
2. Our installation process is as follows:
   1. Extracting necessary executables to installation folder
   2. Setting registry entries
   3. Creating and start Windows Service
3. Telivy Agent is a Windows Service and it is responsible for sending heartbeat log and start Telivy Scanner every hour.
4. The first step for the Telivy Scanner is to reach to the Telivy servers to create an asset in your assessment. It gathers a hostname, OS name and any other machine identifying metadata.
...
3. **C:\\Program Files (x86)\\Telivy\\Telivy Agent\\TelivyScanner.exe: **Performs thr actual scanning