...
<Tabs>
  <Tab title="External Vulnerabilities">
    In this section we scan the publicly accessible services like your websites, external IPs or endpoints that can be accessed via the public internet. Telivy parses your DNS and uses a network mapper to scan these endpoints to infer the existing vulnerabilities.

    The platform then classifies these vulnerabilities as Low, Medium, or High Severity. Note that these classifications are based on the NVD database maintained by the federal government.

    <img alt="External vulnerability findings labelled Low, Medium, and High severity" src="/images/external-vulnerabilities.jpg" />

     </Tab>

  <Tab title="Network Security">
    In this section we scan for weaknesses/vulnerabilities in the network itself. Here are some checks we perform for External Network Security.
    ...
    <img alt="Network Security findings covering email, open ports, and certificates" src="/images/network-security.jpg" />
  </Tab>
</Tabs>
