...
<AccordionGroup>
  ...
  <Accordion title="Processing Webhooks">
    ...
    This approach ensures that your webhook endpoint responds quickly, which prevents timeouts and unnecessary retries. Our webhook requests will time out after 10 seconds.

    ### **HTTP Response Codes**

    - **2xx**: Success - We'll consider the webhook delivered
    - **Other status codes**: Failure - We will retry sending the webhook: the first attempt immediately, the second after a delay
  </Accordion>
